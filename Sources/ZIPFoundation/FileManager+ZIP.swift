...
//            let path = pathEncoding == nil ? entry.path : entry.path(using: pathEncoding!)
            ...
            let encoding = String.Encoding.appropriateEncoding(for: entry.centralDirectoryStructure.fileNameData)
    ...
    public func unzipItem2(at sourceURL: URL, to destinationURL: URL,
                          skipCRC32: Bool = false, allowUncontainedSymlinks: Bool = false,
                           progress: Progress? = nil, automaticallyDetectPathEncoding: Bool, pathEncoding: String.Encoding? = nil) throws {
        let fileManager = FileManager()
        guard fileManager.itemExists(at: sourceURL) else {
            throw CocoaError(.fileReadNoSuchFile, userInfo: [NSFilePathErrorKey: sourceURL.path])
        }
...
extension String.Encoding {
    static func appropriateEncoding(for data: Data) -> String.Encoding? {
        findAppropriateEncoding(for: data)
    }
}

private func findAppropriateEncoding(for data: Data) -> String.Encoding? {
    ...
    if #available(iOS 16.0, *) {
        options[.likelyLanguageKey] = Locale.current.language.languageCode?.identifier ?? "en"
    }
    
    let detectedEncodingRawValue = NSString.stringEncoding(
        for: data,
    ...
    guard detectedEncodingRawValue != 0 else {
        return nil
    }
    
    return String.Encoding(rawValue: detectedEncodingRawValue)
}
